...
if ($_SESSION [loginflag] == 1 && $_SESSION[loginduration]>time()) {
	...
	$db_menu=new DBSql();
	$tmpmenusql='select id,name from menu m,(SELECT distinct(parent_id) parent_id FROM menu m,menu_role mr WHERE m.id=mr.menu_id and mr.role_id='.$_SESSION[loginroleid].') mp where mp.parent_id=m.id;';
